...
class ActionThinkingPursuerAgent:
    def __init__(self,
                 state_dim,
                 action_dim,
                 max_action,
        ...
        # TD3 for predicting opponent's next action
        self.opponent_predictor = SequentialTD3(
            state_dim=state_dim,  # Uses the same state dim as movement agent
            action_dim=action_dim + 1,  # Predicting opponent action + uncertainty
            max_action=max_action,
            history_len=history_len,
            hidden_dim=hidden_dim,
            discount=discount,
    ...
    def sample_action(self, state):
        ...
        pred_opponent_action, uncertainty = self.opponent_predictor.sample_action()
        return mov_action, com_action, pred_opponent_action, uncertainty
    ...
    def select_train_action(self, state):
        ...
        pred_opponent_action, uncertainty = self.opponent_predictor.select_train_action(state)
        return mov_action, com_action, pred_opponent_action, uncertainty

    def select_action(self, state):
        # start = time()
        mov_action = self.movement_agent.select_action(state)
        # print(f"[DEBUG] movement_agent.select_action took {time() - start:.6f} seconds")
        # start = time()
        com_action = self.communication_agent.choose_action(state)
        # print(f"[DEBUG] communication_agent.choose_action took {time() - start:.6f} seconds")
        # start = time()
        pred_opponent_action, uncertainty = self.opponent_predictor.select_action(state)
        # print(f"[DEBUG] opponent_predictor.select_action took {time() - start:.6f} seconds")

        return mov_action, com_action[0], pred_opponent_action, uncertainty
    ...
    def store_buffer(self,
                     ...
                     opponent_action_pred, opponent_action_uncertainty, loss, # Opponent TD3
        ...
        self.opponent_predictor.replay_buffer.add(pursuer_states, [opponent_action_pred, opponent_action_uncertainty], -loss, next_pursuer_states, done)
    ...
    def save(self, save_dir):
        ...
        self.opponent_predictor.save(save_dir, name='opponent_action_predictor')

    def load(self, filename):
        self.communication_agent.load_models(filename)
        self.movement_agent.load(filename)
        self.opponent_predictor.load(filename, name='opponent_action_predictor')